...
function setupAnimation() {
    const container = document.getElementById("overlay-explorer-container")
    const space = document.getElementById("overlay-explorer-space")
    const button = document.getElementById("overlay-explorer-button")

    if (!container || !space || !button) return

    // Set initial state from localStorage if enabled
    const saveState = button.getAttribute("data-olsavestate") === "true"
    const storageKey = "overlayExplorerState"
    let isOpen = false
    ...
    function toggleOverlay(state) {
        ...
        if (isOpen) {
            container.classList.add("active")
            // Small delay to allow display property to take effect before starting opacity transition
            setTimeout(() => {
            ...
            // Wait for transform animation to complete before hiding container
            setTimeout(() => {
                container.classList.remove("active")
